...
export default function AppSidebar() {
  ...
  useEffect(() => {
    ...
    function handleMobileViewChange(event: Event) {
      ...
      // Siempre cerrar menú al cambiar de modo
    ...
    return (
      <aside className="app-sidebar-mobile-mode">
        <div className="app-sidebar-mobile-header">
          ...
          <span className="app-sidebar-mobile-current">{activeItem.label}</span>
        </div>
    ...
    <aside className="app-sidebar">
      <div className="app-sidebar-inner">
        ...
        <nav className="app-sidebar-nav">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} style={navItem(item.href)}>
              {item.label}
            </Link>
          ))}
        </nav>
